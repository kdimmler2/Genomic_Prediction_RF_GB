...
rule all:
    input:
        'validation_data/variants.txt',
        'validation_data/subset_validation_demo.vcf.gz',
        'validation_data/modeling.table',
        'validation_data/raw_tables/',
        'validation_data/modeling_input.txt',

rule get_variants:
    input:
        table = config['training_table'],
    output:
        variants = 'validation_data/variants.txt',
    resources:
        time    = 60,
        mem_mb  = 24000,
        cpus    = 4,
...
rule get_vcf:
    input:
        ...
        variants = 'validation_data/variants.txt',
    output:
        itr_vcf = 'validation_data/subset_validation_demo.vcf.gz',
        itr_tbi = 'validation_data/subset_validation_demo.vcf.gz.tbi'
    resources:
        time    = 60,
        mem_mb  = 24000,
        cpus    = 4,
...
rule gatk_table:
    input:
        validation_vcf = 'validation_data/subset_validation_demo.vcf.gz', 
    output:
        table = 'validation_data/modeling.table', 
    resources:
        time    = 60,
        mem_mb  = 6000,
    shell:
...
checkpoint split_table:
    input:
        table = 'validation_data/modeling.table',
    output:
        split_tables = directory('validation_data/raw_tables/'),
    resources:
        time    = 10,
        mem_mb  = 12000,
    run:
        ...
        output_dir = 'validation_data/raw_tables'
...
rule convert_tables:
    input:
        table = 'validation_data/raw_tables/part_{num}.txt'
    output:
        converted_table = 'validation_data/converted_tables/part_{num}.txt',
    params:
...
def get_file_nums(wildcards):
    ...
    return sorted(expand(
        'validation_data/converted_tables/part_{num}.txt',
        num=PARTS
    ))
...
rule combine_tables:
    ...
    output:
        modeling_input = 'validation_data/modeling_input.txt',
    params:
        directory = 'validation_data/converted_tables/',
    resources:
        time    = 120,
        mem_mb  = 12000,
    run:
