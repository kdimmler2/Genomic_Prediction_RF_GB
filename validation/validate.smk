...
from sklearn.metrics import accuracy_score, confusion_matrix, precision_score, recall_score, ConfusionMatrixDisplay, f1_score, average_precision_score, roc_auc_score, balanced_accuracy_score
...
rule all:
    input:
        expand('results/iteration{itr}/inputs/variants.txt', itr=[str(i) for i in range(1,47)]),
        expand('results/iteration{itr}/inputs/validation.vcf.gz', itr=[str(i) for i in range(1,47)]),
        expand('results/iteration{itr}/inputs/validation.vcf.gz.tbi', itr=[str(i) for i in range(1,47)]),
        expand('results/iteration{itr}/inputs/validation.table', itr=[str(i) for i in range(1,47)]),
        expand('results/iteration{itr}/inputs/raw_tables/', itr=[str(i) for i in range(1,47)]),
        expand('results/iteration{itr}/inputs/validation_input.txt', itr=[str(i) for i in range(1,47)]),
        expand('results/iteration{itr}/f1_score.txt', itr=[str(i) for i in range(1,47)]),
        expand('results/iteration{itr}/confusion_matrix.png', itr=[str(i) for i in range(1,47)]),

rule get_variants:
    input:
        table = '../results/iteration{itr}/inputs/modeling.table', 
    output:
        variants = 'results/iteration{itr}/inputs/variants.txt', 
    resources:
        time    = 60,
        mem_mb  = 24000,
        cpus    = 4,
...
rule get_vcf:
    input:
        ...
        variants = 'results/iteration{itr}/inputs/variants.txt', 
    output:
        itr_vcf = 'results/iteration{itr}/inputs/validation.vcf.gz',
        itr_tbi = 'results/iteration{itr}/inputs/validation.vcf.gz.tbi' 
    resources:
        time    = 60,
        mem_mb  = 24000,
        cpus    = 4,
...
rule gatk_table:
    input:
        itr_vcf = 'results/iteration{itr}/inputs/validation.vcf.gz', 
    output:
        table = 'results/iteration{itr}/inputs/validation.table', 
    resources:
        time    = 60,
        mem_mb  = 24000,
        cpus    = 4,
...
checkpoint split_table:
    input:
        table = 'results/iteration{itr}/inputs/validation.table',
    output:
        split_tables = directory('results/iteration{itr}/inputs/raw_tables/'),
    resources:
        time    = 10,
        mem_mb  = 24000,
        cpus    = 4,
...
rule convert_tables:
    input:
        table = 'results/iteration{itr}/inputs/raw_tables/part_{num}.txt'
    output:
        converted_table = 'results/iteration{itr}/inputs/converted_tables/part_{num}.txt',
    params:
...
def get_file_nums(wildcards):
    ...
    return sorted(expand(
        'results/iteration{itr}/inputs/converted_tables/part_{num}.txt',
        itr=wildcards.itr,
        num=PARTS
    ))
...
rule combine_tables:
    ...
    output:
        validation_input = 'results/iteration{itr}/inputs/validation_input.txt',
    params:
        directory = 'results/iteration{itr}/inputs/converted_tables/',
    resources:
        time    = 120,
        mem_mb  = 24000,
        cpus    = 4,
...
rule run_forest:
    input:
        forest = '../results/iteration{itr}/random_forest_model.pkl',
        table = 'results/iteration{itr}/inputs/validation_input.txt',
       # features = '../X_val.csv',
       # outcomes = '../y_val.csv',
    output:
        f1_score = 'results/iteration{itr}/f1_score.txt',
        confusion_matrix = 'results/iteration{itr}/confusion_matrix.png',
    resources:
        time    = 30,
        mem_mb  = 60000,
        cpus    = 32,
