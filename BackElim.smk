...
rule all:
    input:
        'results/current_iteration/inputs/modeling.table',
        'results/current_iteration/inputs/raw_tables/',
        'results/current_iteration/inputs/modeling_input.txt',
        'results/current_iteration/evaluation_metrics.tsv',
        'results/current_iteration/random_forest_model.pkl',
        'results/current_iteration/feature_importance_scores.txt',
        'results/current_iteration/top_80_features.txt',
        'results/current_iteration/BackElim.vcf.gz',

rule gatk_table:
    input:
        modeling_vcf = 'results/previous_iteration/BackElim.vcf.gz', 
    output:
        table = 'results/current_iteration/inputs/modeling.table', 
    resources:
        time    = 60,
        mem_mb  = 24000,
        cpus    = 4,
...
checkpoint split_table:
    input:
        table = 'results/current_iteration/inputs/modeling.table',
    output:
        split_tables = directory('results/current_iteration/inputs/raw_tables/'),
    resources:
        time    = 10,
        mem_mb  = 24000,
        cpus    = 4,
    run:
        ...
        output_dir = 'results/current_iteration/inputs/raw_tables'
        lines_per_file = 1000
...
rule convert_tables:
    input:
        table = 'results/current_iteration/inputs/raw_tables/part_{num}.txt'
    output:
        converted_table = 'results/current_iteration/inputs/converted_tables/part_{num}.txt',
    params:
...
def get_file_nums(wildcards):
    ...
    return sorted(expand(
        'results/current_iteration/inputs/converted_tables/part_{num}.txt',
        num=PARTS
    ))
...
rule combine_tables:
    ...
    output:
        modeling_input = 'results/current_iteration/inputs/modeling_input.txt',
    params:
        directory = 'results/current_iteration/inputs/converted_tables/',
    resources:
        time    = 120,
        mem_mb  = 24000,
        cpus    = 4,
...
rule random_forest:
    input:
        modeling_input = 'results/current_iteration/inputs/modeling_input.txt',
    output:
        evaluation_metrics = 'results/current_iteration/evaluation_metrics.tsv',
        model = 'results/current_iteration/random_forest_model.pkl',
        importance_scores = 'results/current_iteration/feature_importance_scores.txt',
    resources:
        time= 300,
        mem_mb= 60000,
        nodes= 1,
...
rule top_80_features:
    input:
        feature_importances = 'results/current_iteration/feature_importance_scores.txt',
    output:
        top80 = 'results/current_iteration/top_80_features.txt',
    resources:
        time    = 10,
        mem_mb  = 20000,
    run:
...
rule top_80_variants:
    input:
        ...
        top80 = 'results/current_iteration/top_80_features.txt',
    output:
        first_iteration_vcf = 'results/current_iteration/BackElim.vcf.gz',
    resources:
        time    = 10,
        mem_mb  = 2000,
    shell:
        '''
            ...
            cp results/current_iteration results/temp -r
